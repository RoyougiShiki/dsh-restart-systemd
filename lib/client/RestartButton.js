 ...
 */
export function restartReloadDecision(state, flags) {
 ...
 */
export function RestartButton({ wide, useConnectionState, t }) {
    ...
    const trigger = (_jsx("button", { ref: triggerRef, type: "button", className: "dsh-restart-trigger", style: wide ? triggerStyle : triggerRailStyle, "aria-label": label, "aria-busy": pending || undefined, disabled: pending, title: pending ? t('restart.pendingShort') : label, onClick: () => {
            setPhase({ kind: 'confirming' });
            setOpen(true);
        }, children: pending ? _jsx(SpinnerGlyph, { size: wide ? 16 : 18 }) : _jsx(RestartGlyph, { size: wide ? 16 : 18 }) }));
    const dialog = open && createPortal((_jsxs("div", { style: overlayStyle, role: "presentation", children: [_jsx("div", { style: maskStyle, "aria-hidden": "true", onClick: close }), _jsxs("div", { style: dialogStyle, role: "dialog", "aria-modal": "true", "aria-label": label, children: [phase.kind === 'confirming' && (_jsxs(_Fragment, { children: [_jsx("p", { style: titleStyle, children: t('restart.confirm.title') }), _jsx("p", { style: bodyStyle, children: t('restart.confirm.body') }), _jsxs("div", { style: actionsStyle, children: [_jsx("button", { ref: cancelRef, type: "button", "data-dsh-restart-action": "cancel", style: cancelStyle, onClick: close, children: t('restart.cancel') }), _jsx("button", { ref: proceedRef, type: "button", "data-dsh-restart-action": "proceed", style: proceedStyle, onClick: confirm, children: t('restart.proceed') })] })] })), phase.kind === 'pending' && (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, role: "status", children: [_jsx(SpinnerGlyph, { size: 18 }), _jsxs("div", { children: [_jsx("p", { style: titleStyle, children: t('restart.pending') }), _jsx("p", { style: hintStyle, children: t('restart.pendingHint') })] })] })), phase.kind === 'denied' && (_jsx("p", { style: errorStyle, children: t('restart.denied') })), phase.kind === 'failed' && (_jsxs(_Fragment, { children: [_jsx("p", { style: errorStyle, children: phase.message }), _jsx("p", { style: hintStyle, children: t('restart.failedHint') })] }))] })] })), document.body);
