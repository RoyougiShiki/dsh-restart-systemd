...
const successTitleStyle = { margin: 0, fontSize: 14, fontWeight: 600, color: 'var(--dsw-alias-label-primary)' };
const successSubStyle = { margin: '2px 0 0', fontSize: 12, color: 'var(--dsw-alias-label-secondary)' };
function SpinnerGlyph({ size = 16 }) {
    return (_jsxs("svg", { width: size, height: size, viewBox: "0 0 16 16", fill: "none", "aria-hidden": "true", style: { animation: 'dsh-restart-spin 1s linear infinite' }, children: [_jsx("circle", { cx: "8", cy: "8", r: "6", stroke: "var(--dsw-alias-label-tertiary)", strokeWidth: "2", opacity: "0.35" }), _jsx("path", { d: "M14 8a6 6 0 0 0-6-6", stroke: "var(--dsw-alias-label-secondary)", strokeWidth: "2", strokeLinecap: "round" })] }));
}
function CheckGlyph({ size = 16 }) {
    return (_jsxs("svg", { width: size, height: size, viewBox: "0 0 16 16", fill: "none", "aria-hidden": "true", children: [_jsx("circle", { cx: "8", cy: "8", r: "7", fill: "color-mix(in srgb, var(--dsw-alias-state-success-primary, #3fb950) 18%, transparent)" }), _jsx("path", { d: "M4.5 8.2l2.3 2.3 4.7-5", stroke: "var(--dsw-alias-state-success-primary, #3fb950)", strokeWidth: "1.6", strokeLinecap: "round", strokeLinejoin: "round" })] }));
}
...
export function RestartButton({ wide, t }) {
    ...
    const actionsRef = useRef({ close: () => { }, confirm: () => { } });
    ...
    const trigger = (_jsx("button", { ref: triggerRef, type: "button", className: "dsh-restart-trigger", style: wide ? triggerStyle : triggerRailStyle, "aria-label": label, "aria-busy": phase.kind === 'busy' || undefined, disabled: phase.kind === 'busy', title: phase.kind === 'busy' ? t('restart.busy') : label, onClick: () => {
            ...
            setOpen(true);
        }, children: phase.kind === 'busy' ? _jsx(SpinnerGlyph, { size: wide ? 16 : 18 }) : _jsx(RestartGlyph, { size: wide ? 16 : 18 }) }));
    const dialog = open && createPortal((_jsxs("div", { style: overlayStyle, role: "presentation", children: [_jsx("div", { style: maskStyle, "aria-hidden": "true", onClick: close }), _jsxs("div", { style: dialogStyle, role: "dialog", "aria-modal": "true", "aria-label": label, children: [phase.kind === 'confirming' && (_jsxs(_Fragment, { children: [_jsx("p", { style: titleStyle, children: t('restart.confirm.title') }), _jsx("p", { style: bodyStyle, children: t('restart.confirm.body') }), _jsxs("div", { style: actionsStyle, children: [_jsx("button", { ref: cancelRef, type: "button", "data-dsh-restart-action": "cancel", style: cancelStyle, onClick: close, children: t('restart.cancel') }), _jsx("button", { ref: proceedRef, type: "button", "data-dsh-restart-action": "proceed", style: proceedStyle, onClick: confirm, children: t('restart.proceed') })] })] })), phase.kind === 'busy' && (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, children: [_jsx(SpinnerGlyph, { size: 18 }), _jsxs("div", { children: [_jsx("p", { style: titleStyle, children: t('restart.busy') }), busyAt > 0 && Date.now() - busyAt > 15000 && (_jsx("p", { style: hintStyle, children: t('restart.busySlow') })), busyAt > 0 && Date.now() - busyAt > 5000 && Date.now() - busyAt <= 15000 && (_jsx("p", { style: hintStyle, children: t('restart.busyWait') }))] })] })), phase.kind === 'done' && (_jsxs("div", { style: successCardStyle, role: "status", children: [_jsx(CheckGlyph, { size: 18 }), _jsxs("div", { children: [_jsx("p", { style: successTitleStyle, children: t('restart.done') }), _jsx("p", { style: successSubStyle, children: "DeepSeek Harness" })] })] })), phase.kind === 'denied' && (_jsx("p", { style: errorStyle, children: t('restart.denied') })), phase.kind === 'failed' && (_jsxs(_Fragment, { children: [_jsx("p", { style: errorStyle, children: phase.message }), _jsx("p", { style: hintStyle, children: t('restart.failedHint') })] }))] })] })), document.body);
    ...
    return (_jsxs(_Fragment, { children: [trigger, dialog] }));
}
