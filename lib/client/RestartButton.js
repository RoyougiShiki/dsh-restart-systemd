...
export function RestartButton({ wide, t }) {
  ...
  const actionsRef = useRef({ close: () => {}, confirm: () => {} });
    ...
    if (!wide && railHost) {
        return _jsxs(_Fragment, { children: [createPortal(trigger, railHost), dialog] });
    }
