 ...
 */
export function waitForReconnect(timeoutMs = 45000, initialWaitMs = 8000): Promise<boolean> {
  const started = Date.now()
  const deadline = started + timeoutMs
  return new Promise((resolve) => {
    const probe = async (): Promise<void> => {
      // The restart is scheduled with a ~3s delay, then systemd restarts the
      // unit (RestartSec=3) and the app boots. Probing before that window
      // would resolve instantly against the still-alive process and the
      // "reconnected" card would close before the restart actually happens.
      if (Date.now() < started + initialWaitMs) {
        setTimeout(() => void probe(), 500)
        return
      }
      ...
      try {
        // Strict success probe against the app root — /api/health does not
        // exist (404), and any non-2xx must not count as "reachable".
